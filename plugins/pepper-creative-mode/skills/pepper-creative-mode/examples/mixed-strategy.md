...
Without `pepper-creative-mode`, models tend to favor specific moves — "Rock" is a common default, plausibly because it is listed first and appears often in training data. The `pepper-creative-mode` string breaks this pattern by committing the model to an unpredictable intermediate value before the decision is made. This is the setting where the technique is most clearly worth its cost: the paper measures 98% and 99% reductions in JS divergence on 3-choice and biased 3-choice tasks for QwQ-32B, and runs a dedicated Rock-Paper-Scissors experiment against adversarial bots.

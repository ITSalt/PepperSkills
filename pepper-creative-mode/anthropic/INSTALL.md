...
Проверь три вещи по порядку.

1. **Температура.** При `temperature = 0` или фиксированном seed строка
   перестаёт быть случайной, и скилл превращается в дорогой no-op.
   В экспериментах статьи `T = 0.6`–`1.0`.
2. **Независимость прогонов.** Замеры делаются независимыми вызовами.
   Десять бросков подряд в одном чате — не выборка: модель видит
   предыдущие ответы и начинает «выравнивать» серию. Считай в новых чатах.
3. **Класс модели.** Граница проходит по наличию reasoning, а не по
   размеру: обычная Qwen3-4B с техникой ухудшается на 436 %, а её же
   thinking-версия улучшается на 88 %. На reasoning-моделях
   deepseek-r1 доходит до 3.03 ×10⁻³ при эталоне ГСЧ 1.85 ×10⁻³.

И отдельно: на **честной** монетке техника слабее всего — это
единственный сценарий, где в статье модель стала хуже (QwQ-32B,
2.43 → 3.39 ×10⁻³). Смысл она приобретает на смещённых распределениях.
